...
def calculate_derived_metrics(client_data: Dict[str, str]) -> Dict[str, Union[str, float]]:
    ...
    try:
        ...
        median_ticket = float(client_data.get("median_ticket", 0))
        frequency_new = float(client_data.get("frequency_new", 0))
        prom_contacts_month_new = float(client_data.get("prom_contacts_month_new", 0))
        
        # Calculate monthly values
        monthly_revenue = median_ticket * frequency_new
        monthly_cost = (prom_contacts_month_new * 15) + (frequency_new * 10)
        monthly_profit_margin = monthly_revenue - monthly_cost
        
        # Calculate time to realize savings
        if monthly_profit_margin > 0 and savings > 0:
            months_to_realize_savings = round(savings / monthly_profit_margin, 1)
        else:
            months_to_realize_savings = "N/A"
        ...
        return {
            ...
            "months_to_realize_savings": months_to_realize_savings,
...
def format_client_context(client_data: Dict[str, str], derived_metrics: Dict[str, Union[str, float]]) -> str:
    ...
    for key, value in derived_metrics.items():
        if key == "efficiency_improvement" and value != "N/A":
            context_lines.append(f"{key}: {value}%")
        elif key in ["monthly_revenue", "monthly_cost", "monthly_profit_margin"] and value != "N/A":
            context_lines.append(f"{key}: {value}€")
        elif key == "months_to_realize_savings" and value != "N/A":
            context_lines.append(f"{key}: {value} months")
        else:
            context_lines.append(f"{key}: {value}")
...
4. **Business Impact**: Quantify savings and explain the timeline
5. **Savings Timeline**: Using the months_to_realize_savings value, clearly state: "With these changes, [savings]€ will be saved in [months_to_realize_savings] months"
...
✅ Always include the savings timeline statement for cases WITH changes
...
Business Impact: This optimization generates [savings]€ in total savings. With these changes, [savings]€ will be saved in [months_to_realize_savings] months."
